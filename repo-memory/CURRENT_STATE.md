...
- **Runtime-owned operator/config surface** (2026-04-18 worktree): credentials can be read/written through `orchestrator.py creds`, Telegram operator approval lives in `state/runtime/allowlist.json`, and new repos can be added through `orchestrator.py register-project` instead of hand-editing tracked docs or local config by hand.
